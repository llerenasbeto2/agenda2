...
public function store(Request $request)
{
    ...
    try {
        ...
        DB::rollBack();
        \Log::error('Error al crear facultad: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
        
        // Preservar datos del formulario para Inertia
        $oldInput = $request->except(['_token', 'image_file', 'classrooms']);
        $oldClassrooms = $request->input('classrooms', []);
        
        // Para errores de validación en responsables, etc.
...
private function crearClassrooms(array $classroomsData, faculty $faculty, Request $request): void
{
    ...
    foreach ($classroomsData as $index => $classroomData) {
        ...
        $imageFile = $request->file("classrooms.{$index}.image_file");
        if ($classroomData['image_option'] === 'upload' && $imageFile) {
            $classroom['image_url'] = null;
            $classroom['image_path'] = $imageFile->store('classrooms', 'public');
        ...
        if (!empty($classroomData['responsible'])) {
            ...
            if ($updated === 0) {
                throw new \Exception("El usuario seleccionado como responsable del aula '{$classroomData['name']}' no tiene el rol correcto (rol_id = 2).");
            }
            $responsablesToUpdate[$classroomData['responsible']] = $createdClassroom->id;
        }
    }
    
    // Actualizar todos los responsables de classrooms en batch (redundante ahora, pero ok)
...
public function update(Request $request, faculty $faculty)
{
    ...
    try {
        ...
        DB::rollBack();
        \Log::error("Error al actualizar facultad ID {$faculty->id}: " . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
        
        // Preservar datos del formulario para Inertia (similar para edit, asumiendo vista similar)
        $oldInput = $request->except(['_token', 'image_file', 'classrooms']);
        $oldClassrooms = $request->input('classrooms', []);
        
        $faculty->load('classrooms'); // Cargar datos originales para merge si es necesario
...
): void
{
    foreach ($responsablesIds as $responsableId) {
        // Limpiar faculties (excepto la actual si es su responsable) - solo numéricos
        ...
        // Limpiar classrooms (excepto los que se están actualizando con este responsable) - solo numéricos
...
private function gestionarResponsableFacultad(int $facultyId, ?int $responsableId): void
{
    if ($responsableId) {
        ...
        $updated = User::where('id', $responsableId)
            ->where('rol_id', 3)
            ->update(['responsible_id' => $facultyId]);
        if ($updated === 0) {
            throw new \Exception("El usuario seleccionado como responsable de la facultad no tiene el rol correcto (rol_id = 3).");
        }
    } else {
 ...
 */
private function procesarClassrooms(array $classroomsData, faculty $faculty, Request $request): void
{
    foreach ($classroomsData as $index => $classroomData) {
        $classroom = [
            'name' => $classroomData['name'],
...
private function gestionarResponsableClassroom(int $classroomId, ?int $responsableId): void
{
    if ($responsableId) {
        ...
        $updated = User::where('id', $responsableId)
            ...
            ->update(['responsible_id' => $classroomId]);
        if ($updated === 0) {
            throw new \Exception("El usuario seleccionado como responsable del aula no tiene el rol correcto (rol_id = 2).");
        }
    } else {
        // Si no hay responsable, limpiar todos
        User::where('responsible_id', $classroomId)

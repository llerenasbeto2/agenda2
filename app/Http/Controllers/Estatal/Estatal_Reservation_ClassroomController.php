...
class Estatal_Reservation_ClassroomController extends Controller
{
public function index(Request $request)
    {
        ...
        // Si el usuario tiene un responsible_id, filtrar por la facultad correspondiente
        if ($user && $user->responsible_id) {
            $query->where('faculty_id', $user->responsible_id);
        }
        ...
        return Inertia::render('Admin/Estatal/Dashboard', [
            'reservaciones' => $reservaciones,
            'faculties' => Faculty::when($user->responsible_id, function ($query) use ($user) {
                return $query->where('id', $user->responsible_id);
            }, function ($query) {
                return $query;
            })->get(['id', 'name']),
            'classrooms' => Classroom::when($user->responsible_id, function ($query) use ($user) {
                return $query->where('faculty_id', $user->responsible_id);
            }, function ($query) {
                return $query;
            })->get(['id', 'name']),
...
return redirect()->route('admin.estatal.dashboard.index')

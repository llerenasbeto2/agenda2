...
watch([selectedStatus, selectedDate, selectedMunicipality, () => filters.value.event_title], () => {
  currentPage.value = 1;
});
...
const openCommentModal = (reservacion) => {
  selectedReservation.value = reservacion;
...
const changeStatus = (reservacion, newStatus) => {
  const form = useForm({
    id: reservacion.id,
    status: newStatus,
  });

  form.put(route('admin.general.events.cambiarEstado'), {
    onSuccess: () => {
      reservacion.status = newStatus;
      showStatusDropdown.value = null;
    },
  });
};
...
const formatFecha = (fecha) => {
  if (!fecha) return '';
  try {
    const dateStr = fecha.slice(0, 16);
...
<template>
  ...
  <AdminGeneralLayout>
    ...
    <CityButtonGroup @city-selected="handleCitySelection" />
        ...
        <div class="mb-4 bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <form method="get" :action="route('admin.general.dashboard')" @submit.prevent="search">
                <div class="flex items-center">
                  <input
          ...
          <template #default="{ paginatedItems }">
            <div class="overflow-x-auto">
              <table class="min-w-full bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg overflow-hidden mt-4">
                ...
                <tbody>
                  <tr
                    v-for="reservacion in paginatedItems"
                    :key="reservacion.id"
                    class="border-b dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600"
                  >
                    ...
                    <td class="py-2 px-4 text-gray-800 dark:text-gray-200">
                      {{ reservacion.category ? reservacion.category.name : 'Clase' }}
                    </td>
                    <td class="py-2 px-4 text-gray-800 dark:text-gray-200">
                      {{ formatFecha(reservacion.start_datetime) }}
                    </td>
                    <td class="py-2 px-4 text-gray-800 dark:text-gray-200">
                      {{ reservacion.classroom ? reservacion.classroom.name : 'Aula DGRE' }}
                    </td>
                    <td class="py-2 px-4 flex gap-2">
                      <button
                        @click="openViewModal(reservacion)"
                      ...
                      <button
                        @click="openEditModal(reservacion)"
                      ...
                      <button
                        @click="eliminarReservacion(reservacion.id)"
                      ...
                      <button
                        @click="openCommentModal(reservacion)"
                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold p-2 rounded-full text-sm"
                        title="Comentarios"
                      >
                        <svg
                      ...
                      <button
                        @click="openPaymentModal(reservacion)"
    ...
    <!-- Modales existentes -->
    <AlertAdeudo :show="showIngresosModal" @close="closeIngresosModal" />
    <AlertCuenta :show="showIngresosModalcuenta" @close="closeIngresosModalcuenta" />
    <AlertCondiciones :show="showIngresosModalcondiciones" @close="closeIngresosModalcondiciones" />

...
onMounted(() => {
  console.log('Reservation data:', JSON.stringify(props.reservation, null, 2));
});
...
const statusColor = computed(() => {
  ...
  if (['Aprobado', 'Realizado'].includes(status)) return 'bg-green-100 text-green-800';
...
const formattedCost = computed(() => {
  if (!props.reservation || props.reservation.cost == null) return '$0.00';
  
  try {
    const cost = Number(props.reservation.cost);
    if (isNaN(cost)) {
      console.error('Costo inválido:', props.reservation.cost);
      return '$0.00';
    }
    return `$${cost.toFixed(2)}`;
  } catch (error) {
    console.error('Error al formatear costo:', error, props.reservation.cost);
    return '$0.00';
  }
});
...
const isPaid = computed(() => {
  ...
  const cost = Number(props.reservation.cost);
  return (!isNaN(cost) && cost > 0) || props.reservation.is_paid === 1 || props.reservation.is_paid === true;
});
</script>
            ...
            <div>
              ...
              <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg space-y-3">
                <div>
                  ...
                  <p class="text-gray-800 dark:text-gray-200 font-medium">{{ reservation.event_title || 'No especificado' }}</p>
                </div>
                <div>
                  <p class="text-sm text-gray-500 dark:text-gray-400">Estado</p>
                  <span :class="[statusColor, 'px-2 py-1 rounded-full text-xs font-semibold inline-block']">
                    {{ reservation.status || 'No especificado' }}
                  </span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p class="text-gray-800 dark:text-gray-200">{{ reservation.attendees || 'No especificado' }}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg space-y-3">
                <div>
                  ...
                  <p class="text-gray-800 dark:text-gray-200 font-medium">{{ reservation.full_name || 'No especificado' }}</p>
                </div>
            ...
            <div>
              ...
              <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg space-y-3">
                <div>
                  ...
                  <p class="text-gray-800 dark:text-gray-200">{{ formattedCost }}</p>
                </div>
                <div>
                  <p class="text-sm text-gray-500 dark:text-gray-400">Estado de Pago</p>
                  <p class="text-gray-800 dark:text-gray-200">{{ isPaid ? 'Pagado' : 'No Pagado' }}</p>
                </div>
